...
function loadModule(req, service) {
  try {
    ...
    if (servicePackage.btnNode) {
      serviceModule += '/' + servicePackage.btnNode
    }
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './btn-node.json')
  fullConfig.services = setupServices(require, servicesPath, options.config)

  let node = new BtnNode(fullConfig)
  registerExitHandlers(process, node)
  node.on('ready', () => node.log.info('BtnNode ready'))
