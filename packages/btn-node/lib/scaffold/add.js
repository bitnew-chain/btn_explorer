...
async function add(options) {
  ...
  let btnConfigPath = path.resolve(configPath, 'btn-node.json')
  let packagePath = path.resolve(configPath, 'package.json')

  let existences = await Promise.all([exists(btnConfigPath), exists(packagePath)])
  if (!existences[0] || !existences[1]) {
    throw new Error('Directory does not have a btn-node.json and/or package.json file.')
  }
  ...
  for (let service of services) {
    ...
    await addConfig(btnConfigPath, serviceName)
  }
}
